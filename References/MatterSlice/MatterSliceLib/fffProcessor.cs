...
namespace MatterHackers.MatterSlice
{
	...
	public class FffProcessor
	{
		...
		public bool SetTargetStream(Stream stream)
		{
			gcodeExport.SetOutputStream(stream);
			{
				gcodeExport.WriteComment("Generated with MatterSlice {0}".FormatWith(ConfigConstants.VERSION));
			}
		...
		public bool LoadStlStream(Stream stream)
		{
			timeKeeper.Restart();
			if (!SimpleMeshCollection.LoadModelFromStream(simpleMeshCollection, stream, config.ModelMatrix))
			{
				return false;
			}
		...
		public PrintData Finalize()
		{
			...
			var data = gcodeExport.Finalize(maxObjectHeight, config.TravelSpeed, config.EndCode);
